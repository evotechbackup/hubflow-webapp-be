...
const profitNLossItemSchema = new mongoose.Schema(
  {
    type: {
      type: String,
      required: true,
...
const profitNLossSchema = new mongoose.Schema(
  {
    id: {
      type: String,
    },
...
profitNLossSchema.index({ organization: 1, startDate: 1, endDate: 1 });
profitNLossSchema.index({ organization: 1, createdAt: -1 });

profitNLossSchema.pre('save', async function (next) {
  ...
  const srNo =
    (await mongoose.model('ProfitNLoss').countDocuments({
  ...
  // Format the ID as PNL-YY-MM-DD-SRNo
  this.id = `PNL-${year}-${month}-${day}-${paddedSrNo}`;

  return next();
});

module.exports = mongoose.model('ProfitNLoss', profitNLossSchema);
